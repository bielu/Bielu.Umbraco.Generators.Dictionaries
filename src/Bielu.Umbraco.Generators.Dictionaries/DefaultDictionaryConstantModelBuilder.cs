...
public class DefaultDictionaryConstantModelBuilder(ILocalizationService localizationService, IOptionsMonitor<ModelsBuilderSettings> config, IHostingEnvironment hostingEnvironment, IShortStringHelper shortStringHelper, ILogger<DefaultDictionaryConstantModelBuilder> logger)
    : IDictionaryConstantModelBuilder
{
    public async Task BuildAsync(IEnumerable<IDictionaryItem> dictionaryItems)
    {
        ...
        {
            logger.LogWarning("ModelsBuilder is not in SourceCodeAuto or SourceCodeManual mode, skipping dictionary model generation");
        }
        ...
        var dictionaryConstantModelFilePath =
            Path.Combine(modelsDirectory, $"Dictionaries.generated.cs");
    ...
    public string GetModelsNamespace() =>
        ...
        string.IsNullOrWhiteSpace(config.CurrentValue.ModelsNamespace)
    ...
    private async Task<string> GenerateDictionaryConstantModelText(IEnumerable<IDictionaryItem> dictionaryItems)
    {
        ...
        dictionaryConstantModelText.AppendLine(CultureInfo.InvariantCulture, $"public static class Dictionaries");
    ...
    private static void AddResharperDisableCommentEndFile(StringBuilder dictionaryConstantModelText) => dictionaryConstantModelText.AppendLine(CultureInfo.InvariantCulture, $"#pragma warning restore IDE1006 // Naming Styles");

    private static void AddResharperDisableCommentStartFile(StringBuilder dictionaryConstantModelText)
    {
        dictionaryConstantModelText.AppendLine(CultureInfo.InvariantCulture, $"#pragma warning disable IDE1006 // Naming Styles");
        dictionaryConstantModelText.AppendLine(CultureInfo.InvariantCulture, $"// ReSharper disable InconsistentNaming");
    }
...
#pragma warning restore CA1822
    {
        ...
        dictionaryConstantModelText.Append(CultureInfo.InvariantCulture, $"/// This file is autogenerated by Bielu.Umbraco.Generators.Dictionaries ");
        if (version != null)
        {
            dictionaryConstantModelText.Append(CultureInfo.InvariantCulture,$"v{version.Major}.{version.Minor}.{version.Build}");
        }
        dictionaryConstantModelText.AppendLine();
        dictionaryConstantModelText.AppendLine(CultureInfo.InvariantCulture, $"/// </auto-generated>");
        dictionaryConstantModelText.AppendLine(CultureInfo.InvariantCulture, $"//------------------------------------------------------------------------------");
    ...
    private async Task GenerateDictionaryAsync(StringBuilder stringBuilder, IDictionaryItem dictionary, int lvl = 0)
    {
        ...
        stringBuilder.AppendLine(CultureInfo.InvariantCulture, $"public static class {ToClassName(dictionary.ItemKey)}");
        AppendSpacer(stringBuilder, lvl + 1);
        stringBuilder.AppendLine(CultureInfo.InvariantCulture, $"{{");
        AppendSpacer(stringBuilder, lvl + 2);
        stringBuilder.AppendLine(CultureInfo.InvariantCulture, $"public const string Alias = \"{dictionary.ItemKey}\";");
        AppendSpacer(stringBuilder, lvl + 2);
        stringBuilder.AppendLine(CultureInfo.InvariantCulture, $"public static readonly Guid Key = new Guid(\"{dictionary.Key}\");");
        AppendSpacer(stringBuilder, lvl + 2);
    ...
    private string ToClassName(string dictionaryItemKey) => shortStringHelper.CleanString(dictionaryItemKey, CleanStringType.ConvertCase | CleanStringType.PascalCase);
